...
class PersonalDetailsFragment : ScopedFragment(), KodeinAware {
    override val kodein by lazy { (activity?.applicationContext as KodeinAware).kodein }
    lateinit var mBinding: PersonalDetailsFragmentBinding
    ...
    override fun onCreateView(
        ...
        if (!::mBinding.isInitialized) {
            mBinding = PersonalDetailsFragmentBinding.inflate(inflater, container, false).apply {
    ...
    inner class ClickHandler {
        fun onClickNext() {
            findNavController().navigate(R.id.take_selfie_fragment)
        }
    }
